class Public::RestaurantsController < ApplicationController
  ...
  def show
    ...
    @restaurant_reviews = params[:restaurant_tag_id].present? ? RestaurantTag.find(params[:restaurant_tag_id]).restaurant_reviews.where(restaurant_id: @restaurant).page(params[:page]) : RestaurantReview.where(restaurant_id: @restaurant).page(params[:page])
  end

  private

  def restaurant_params
    params.require(:restaurant).permit(:name, :address, :latitude, :longitude, :country_id, :customer_id)
  end
end
